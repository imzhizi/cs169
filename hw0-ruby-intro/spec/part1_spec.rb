...
describe 'Ruby intro part 1' do
  describe '#sum' do
    ...
    it 'retuprns correct sum [20 points]', points: 20 do
  ...
  describe '#sum_to_n' do
    ...
    it 'returns true when any two elements sum to the second argument [30 points]', points: 30 do
      expect(sum_to_n?([1, 2, 3, 4, 5], 5)).to be_truthy # 2 + 3 = 5
      expect(sum_to_n?([3, 0, 5], 5)).to be_truthy # 0 + 5 = 5
      expect(sum_to_n?([-1, -2, 3, 4, 5, -8], -3)).to be_truthy # handles negative sum
      expect(sum_to_n?([-1, -2, 3, 4, 5, -8], 12)).to be_falsey # 3 + 4 + 5 = 12 (not 3 elements)
      expect(sum_to_n?([-1, -2, 3, 4, 6, -8], 12)).to be_falsey # no two elements that sum
    end
